...
class EventArchiveComponent extends Component
{
    ...
    public function render()
    {
        ...
        if($this->page <= 1){
            $titleEn = 'Events';
            $titleDh = 'ހަރަކާތްތައް';
        } else {
            $titleEn = 'Events' . ' - Page ' .$this->page;
            $titleDh = 'ހަރަކާތްތައް' . ' - ސަފްހާ ' .$this->page;
        }

        return view('livewire.archives.event-archive-component', [
            'data' => Event::query()
            ->search($this->search)
