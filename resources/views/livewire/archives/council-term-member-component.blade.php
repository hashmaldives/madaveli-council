                ...
                <div class="tab-pane fade {{ $key == 0 ? 'show active' : '' }}" id="t{{$key}}" role="tabpanel" aria-labelledby="t{{$key}}-tab">
                    <div class="term-details">
                        <div class="lang-{{ $language }} visible">
                            <h5 class="term-title">{{ $language == 'dh' ?  $parentItem->title_dh . 'ގެ މެމްބަރުން - ' . Carbon\Carbon::parse($parentItem->term_start_date)->translatedFormat('d M Y') . ' އިން ' . Carbon\Carbon::parse($parentItem->term_end_date)->translatedFormat('d M Y') . ' އަށް'  : $parentItem->title_en . ' Members - From ' . Carbon\Carbon::parse($parentItem->term_start_date)->translatedFormat('d M Y') . ' To ' . Carbon\Carbon::parse($parentItem->term_end_date)->translatedFormat('d M Y') }}</h5>
                        </div>
                    </div>
                    <div class="members-container text-center">
                        @foreach($loop->council_members as $item )
                            @if($item->pivot->level == 1)
                            <!--  MEMBER ITEM START  -->
                            <div class="member-item level{{$item->pivot->level}}" wire:key="search-{{ $item->id }}">
                                @if($parentItem->link_to_member_profile == 1) <a href="{{ route('single-council-member', $item->slug ) }}"> @endif
                                    <div class="profile-picture-container d-flex justify-content-center">
                                        <div class="profile-picture zoh">
                                            ...
                                            @endphp
                                            <img src="{{ $image }}">
                                            <div class="political-party-tag lang-{{ $language }} visible"><span style="background-color: {{ $item->political_party->color }};">{{ $language == 'dh' ? $item->political_party->title_dh : $item->political_party->title_en }}</span></div>
                                        </div>
                                    </div>
                                    <!--thumbnail-->
                                    <div class="text">
                                ...
                                @if($parentItem->link_to_member_profile == 1) </a> @endif
                            </div>
                            <!--  MEMBER ITEM END  -->
                    ...
                    <div class="members-container text-center">
                        @foreach($loop2->council_members as $item )
                    ...
                    <div class="members-container text-center">
                        @foreach($loop3->council_members as $item )
