            ...
            <div class="row">
                <div class="col-lg-3">
                    <div class="sidebar">
                        ...
                        <div class="widget">
                            <div class="widget-container">
                                <div id="statusFilterAccordion" class="resource-filter-accordion">
                                    <a class="" type="button" data-toggle="collapse" data-target="#statusFilter" aria-expanded="true" aria-controls="brandFilter">
                                        <h6 class="widget-title with-bg text-left overflow-auto">
                                                <span class="lang-{{ $language }} visible text-light {{ $language == 'dh' ? 'float-right' : 'float-left' }}">{{ $language == 'dh' ? 'ފިލްޓަރ ބައި ސްޓޭޓަސް' : 'Filter by Status' }}</span>
                                            <i class="fas fa-chevron-down {{ $language == 'dh' ? 'float-left' : 'float-right' }}"></i>
                                        </h6>
                                    </a>
                                    <div id="statusFilter" class="resource-filter collapse show" aria-labelledby="headingOne" data-parent="#statusFilterAccordion">
                                        <div class="widget-content">
                                            <div>
                                                @if ($selectedStatus)
                                                    <div class="clear-filter">
                                            ...
                                            @foreach( $statuses as $item )
                                                <label class="form-check-label control control--checkbox {{ $language == 'dh' ? 'rtl' : 'ltr' }}" wire:key="{{ $item->id }}">
                                                    <input type="checkbox" wire:model="selectedStatus" class="form-check-input"  value="{{ $item->id }}">
                                                    <div class="control__indicator"></div>
                                                    <span class="label">
                                                        <div class="lang-{{ $language }} visible"><p class="mb-0">{{ $language == 'dh' ? $item->title_dh : $item->title_en }}</p></div>    
                                                    </span>
