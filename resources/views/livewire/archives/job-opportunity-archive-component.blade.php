                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-6 col-md-6 col-12">
                            <a href="{{ route($resourceRoute, $item->id) }}">
                                <div class="publication-resource-item {{ $item->application_deadline->isPast() == 1 ? 'inactive' : 'active' }}">
                                    <div>
                                        <div class="title">
                                        ...
                                        <div class="meta">
                                            <div class="row">
                                                <div class="col-12 pb-2">
                                                    <p class="lang-{{ $language }} visible">
                                                        <span><i class="far fa-clock text-color-primary-dimm"></i><span class="pl-2 pr-2">{{ $item->created_at->diffForHumans() }}</span>-&nbsp;</span>
                                                        <span class="">{{ $language == 'dh' ? 'މުއްދަތު ހަމަވަނީ:' : 'Deadline:' }}</span>
                                                        <span class="pr-2 pl-2 text-grey">{{ Carbon\Carbon::parse($item->application_deadline)->translatedFormat('d M Y - H:i A') }}</span>
                                                    </p>
                                                </div>
                                                <div class="col-12">
                                                    <div class="d-flex justify-content-end">
                                                        <p class="resource-tag mr-2 lang-{{ $language }} visible {{ $item->application_deadline->isPast() == 1 ? 'inactive' : 'active' }}">
                                                            @if($item->application_deadline->isPast() == 1)
                                                                <span>{{ $language == 'dh' ? 'މުއްދަތު ހަމަވެފައި' : 'Expired' }}</span>
                                                            @else
                                                                <span>{{ $language == 'dh' ? 'އެކްޓިވް' : 'Active' }}</span>
                                                            @endif
                                                        </p>
                                                        <p class="resource-tag lang-{{ $language }} visible">
                                                            <i class="fas fa-tag"></i><span>{{ $language == 'dh' ? $item->section_dh : $item->section_en}}</span>
                                                        </p>
                                                    </div>
                                                </div>
                                            </div>
