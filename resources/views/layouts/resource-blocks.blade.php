...
@endif
            @switch($item->layout)
                ...
                @case('containerlayout')
                    <div style="height: 100%;" class="block-container d-flex {{ $item->attributes->align_content }}">
                        <div class="row {{ $item->attributes->align_content }}">
                            @foreach ($item->attributes->section_elements as $item)
                                @include('layouts/container-blocks')
                            @endforeach
                        </div><!--row-->
                    </div>
                @break
                @case('islandpopulationlayout')
                    <div style="height: 100%;" class="block-container d-flex align-items-center">
                        <div class="table-responsive">
                            <p>{{ $language == 'dh' ? 'އެންމެ ފަހުން އަދާހަމަ ކުރެވުނީ:' : 'Last Updated At:' }} {{ $language == 'dh' ? ThaanaDate::format('j M Y - H:i', Carbon\Carbon::parse(nova_get_setting('updated_at'))->timestamp ) : Carbon\Carbon::parse(nova_get_setting('updated_at'))->format('d M Y - H:i') }}</p>
